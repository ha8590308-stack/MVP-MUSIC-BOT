const {const { Client, GatewayIntentBits, REST, Routes, SlashCommandBuilder, EmbedBuilder, PermissionFlagsBits, ActionRowBuilder, ButtonBuilder, ButtonStyle } = require('discord.js');
...
client.on('messageCreate', async message => {
    ...
    // أمر "توب" بالشات (مع إضافة الشعار واسم MVP فوق في اليسار تماماً مثل الصورة المطلوبة)
...
client.login(TOKEN);
 Client, GatewayIntentBits, REST, Routes, SlashCommandBuilder, EmbedBuilder, PermissionFlagsBits, ActionRowBuilder, ButtonBuilder, ButtonStyle } = require('discord.js');
const express = require('express');
...
client.on('messageCreate', async message => {
    ...
    // أمر "توب" بالشات (بالتصميم السابق والمطلوب بدقة: سطر واحد لكل شخص مع الأيقونات والنقاط والخط الأصفر)
    if (message.content === 'توب') {
        if (userPoints.size === 0) {
            return message.reply('لا توجد أي نقاط مسجلة حتى الآن!');
        }
        ...
        sortedUsers.forEach(([userId, points], index) => {
            ...
            // العودة للتصميم السابق تماماً (سطر واحد لكل عضو مرتب ومنظم)
            description += `${rankEmoji} <@${userId}> -- **${points}** نقطة\n`;
        });
        ...
        const topEmbed = new EmbedBuilder()
            .setColor(0xF1C40F) // اللون الأصفر المميز للإطار الجانبي تماماً مثل الصورة المطلوبة
            .setTitle('🏆 قائمة صدارة الترتيب')
            .setDescription(description)
            .setTimestamp();
