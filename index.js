const {const { Client, GatewayIntentBits, REST, Routes, SlashCommandBuilder, EmbedBuilder, PermissionFlagsBits, ActionRowBuilder, ButtonBuilder, ButtonStyle, StringSelectMenuBuilder, AttachmentBuilder } = require('discord.js');
...
client.login(TOKEN);
 Client, GatewayIntentBits, REST, Routes, SlashCommandBuilder, EmbedBuilder, PermissionFlagsBits, ActionRowBuilder, ButtonBuilder, ButtonStyle, StringSelectMenuBuilder, AttachmentBuilder } = require('discord.js');
const { createCanvas } = require('canvas');
const express = require('express');
...
// دالة رسم عجلة الروليت الدائرية بالـ Canvas تماماً مثل الصورة المطلوبة
async function createWheelImage(players, clientInstance, guildId, selectedPlayerId = null) {
    const width = 500;
    const height = 500;
    const canvas = createCanvas(width, height);
    ...
    // رسم السهم الجانبي الأيمن تماماً مثل الصورة
    ctx.beginPath();
    ctx.moveTo(width - 10, centerY - 15);
    ctx.lineTo(width, centerY);
    ctx.lineTo(width - 10, centerY + 15);
...
client.on('interactionCreate', async interaction => {
    ...
    else if (commandName::resetallpoints) { // تم التصحيح
    }
    else if (commandName === 'resetallpoints') {
        if (!isStaff(interaction.member)) return interaction.reply({ content: 'للإشراف فقط', ephemeral: true });
        userPoints.clear();
